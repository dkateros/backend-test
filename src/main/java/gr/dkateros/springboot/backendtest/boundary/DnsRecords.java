...
public class DnsRecords {
	...
	public ResponseEntity<?> create(@RequestBody DnsRecord dnsRecord, UriComponentsBuilder ucBuilder) {
		if(dnsRecord.getId() != null) {
			return new ResponseEntity<String>("ids are autogenerated!", HttpStatus.BAD_REQUEST);
		}
		dao.save(dnsRecord);
		HttpHeaders headers = new HttpHeaders();
		headers.setLocation(ucBuilder.path("/rs/dnsrecord/{id}").buildAndExpand(dnsRecord.getId()).toUri());
		launchPythonScript();
		return new ResponseEntity<DnsRecord>(dnsRecord, headers, HttpStatus.OK);
	}
	...
	@RequestMapping(value = "/dnsrecord", method = RequestMethod.GET, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> all() {
		System.out.println("ALL with headers");
		List<DnsRecord> records = dao.findAll();
		HttpHeaders headers = new HttpHeaders();
		...
		return new ResponseEntity<List<DnsRecord>>(records, headers, HttpStatus.OK);
	}
